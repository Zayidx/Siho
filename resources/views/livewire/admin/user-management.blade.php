                        ...
                        @forelse ($users as $index => $user)
                            <tr wire:key="{{ $user->id }}">
                                ...
                                <td class="text-center">
                                    <img src="{{ $user->foto_url ?: 'https://placehold.co/100x100/6c757d/white?text=' . strtoupper(substr($user->username, 0, 1)) }}" alt="{{ $user->username }}" class="table-img">
                                </td>
                                <td>{{ $user->username }}</td>
                                <td>{{ $user->email }}</td>
                                <td>
                                    @if ($user->role)
                                        <span class="badge {{ $user->role->name == 'superadmin' ? 'bg-light-info' : 'bg-light-success' }}">{{ $user->role->name }}</span>
                                    @else
                    ...
                    <form wire:submit.prevent="store">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="password" class="form-control @error('password') is-invalid @enderror" id="password" wire:model="password" placeholder="{{ $userId ? 'Kosongkan jika tidak diubah' : '' }}">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="file" accept="image/jpeg,image/png,image/webp" class="form-control @error('foto') is-invalid @enderror" id="foto" wire:model="foto" wire:key="foto-{{ (int)$isModalOpen }}">
