    ...
    <div class="card mb-3">
        <div class="card-body">
            <div class="row g-2 align-items-center">
                ...
                <div class="col">
                    <input type="search" class="form-control" placeholder="Cari nama/email/pesan..." wire:model.live.debounce.400ms="search">
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card">
        <div class="card-body">
            <div class="d-flex justify-content-end mb-3">
                <a href="{{ route('admin.contacts.export', ['status' => $status, 'search' => $search, 'start' => $startDate, 'end' => $endDate]) }}" class="btn btn-sm btn-outline-secondary">Export CSV</a>
            </div>
                    ...
                    <tbody>
                        @forelse($items as $idx => $m)
                            <tr>
                                ...
                                <td>{{ $m->name }}</td>
                                <td>{{ $m->email }}</td>
                                <td class="text-truncate" style="max-width: 360px;" title="{{ $m->message }}">{{ $m->message }}</td>
                                ...
                                <td>
                                    <div class="d-inline-flex gap-1">
                                        <button class="btn btn-sm btn-outline-primary" wire:click="view({{ $m->id }})">Lihat</button>
                                        @if(!$m->read_at)
                                            <button class="btn btn-sm btn-outline-success" wire:click="markRead({{ $m->id }})">Tandai Dibaca</button>
                                        @endif
                                        <button class="btn btn-sm btn-outline-danger" wire:click="delete({{ $m->id }})">Hapus</button>
                                    </div>
                                </td>
                            </tr>
                        @empty
