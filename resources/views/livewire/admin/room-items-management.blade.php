                ...
                <div class="card-body d-flex flex-column p-3">
                    ...
                    <div class="row g-2 mb-2">
                        <div class="col-12">
                            ...
                            <select id="roomTypeFilter" class="form-select form-select-sm" wire:model.live="roomTypeId">
            ...
            <div class="card shadow-sm h-100">
                <div class="card-body d-flex justify-content-center align-items-center text-center text-muted">
                    <div>
                        ...
                        <p>Pilih kamar di sebelah kiri untuk melihat, menambah, <br>atau mengubah data inventori barang.
                        </p>
                    </div>
                </div>
            </div>
            @else
            <div class="card shadow-sm" x-data="{ activeTab: (localStorage.getItem('inv.tab')||'items') }"
                x-init="$watch('activeTab', v => localStorage.setItem('inv.tab', v))">
                <div class="card-header bg-white d-sm-flex justify-content-between align-items-center sticky-top border-bottom p-3"
                    ...
                    <h5 class="card-title mb-2 mb-sm-0">
                        <i class="bi bi-boxes me-2 text-primary"></i>Inventori Kamar <strong class="text-primary">No.
                    ...
                    <div x-show="activeTab === 'items'" x-transition>
                        <div class="card @if($editId) border-primary @else border-light @endif mb-4">
                                ...
                                <div class="row g-3">
                                    <div class="col-md-4">
                                        ...
                                        <input type="text" class="form-control @error('name') is-invalid @enderror"
                                            wire:model="name" placeholder="mis. Handuk" @disabled(!$canEdit)>
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        <input type="number" min="0"
                                            ...
                                            wire:model="quantity" placeholder="mis. 2" @disabled(!$canEdit)>
                                        @error('quantity') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                    </div>
                                    <div class="col-md-3">
                                        <label class="form-label">Satuan</label>
                                        <input type="text" class="form-control @error('unit') is-invalid @enderror"
                                            list="unitsOptions" wire:model="unit" placeholder="mis. buah" @disabled(!$canEdit)>
                                    ...
                                    <div class="col-md-2 d-grid align-self-end">
                                        ...
                                        <button class="btn btn-success" wire:click="update"
                                            wire:loading.attr="disabled" @disabled(!$canEdit)><i class="bi bi-save me-1"></i> Simpan</button>
                                        @else
                                        <button class="btn btn-primary" wire:click="add" wire:loading.attr="disabled" @disabled(!$canEdit)><i
                                ...
                                <div class="d-flex flex-wrap justify-content-between align-items-center mt-3 gap-2">
                                    <div>
                                        ...
                                        <button class="btn btn-sm btn-outline-secondary"
                                            wire:click.prevent="fillPreset('Kasur')" @disabled(!$canEdit)>Kasur</button>
                                        <button class="btn btn-sm btn-outline-secondary"
                                            wire:click.prevent="fillPreset('Handuk')" @disabled(!$canEdit)>Handuk</button>
                                        <button class="btn btn-sm btn-outline-secondary"
                                            wire:click.prevent="fillPreset('Sabun')" @disabled(!$canEdit)>Sabun</button>
                                        <button class="btn btn-sm btn-outline-secondary"
                                            wire:click.prevent="fillPreset('Shampoo')" @disabled(!$canEdit)>Shampoo</button>
                                    </div>
                                    @if($editId)
                                    <button class="btn btn-sm btn-link text-danger" wire:click="cancel">Batal
                                        Ubah</button>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="card-footer bg-primary-subtle text-primary-emphasis small">
                                ...
                                <strong>{{ $name }}</strong>. Klik "Batal Ubah" untuk kembali menambah item baru.
                            </div>
                            @endif
                        </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover table-bordered table-striped align-middle">
                                <thead class="table-light">
                                    <tr>
                                        <th><a href="#" class="text-decoration-none text-dark fw-bold"
                                                wire:click.prevent="sortBy('name')">Nama Item @if($sortField==='name')<i
                                                    class="bi bi-caret-{{ $sortDir==='asc'?'up':'down' }}-fill"></i>@endif</a>
                                        </th>
                                        <th class="text-center" style="width: 180px;"><a href="#"
                                                class="text-decoration-none text-dark fw-bold"
                                                wire:click.prevent="sortBy('quantity')">Jumlah
                                                @if($sortField==='quantity')<i
                                                    class="bi bi-caret-{{ $sortDir==='asc'?'up':'down' }}-fill"></i>@endif</a>
                                        </th>
                                        <th style="width: 150px;"><a href="#"
                                                class="text-decoration-none text-dark fw-bold"
                                                wire:click.prevent="sortBy('unit')">Satuan @if($sortField==='unit')<i
                                                    class="bi bi-caret-{{ $sortDir==='asc'?'up':'down' }}-fill"></i>@endif</a>
                                        </th>
                                ...
                                <tbody>
                                    ...
                                    <tr wire:key="item-{{ $it->id }}">
                                        ...
                                        <td>
                                            <div class="input-group input-group-sm">
                                                <button class="btn btn-outline-secondary"
                                                    wire:click="decrement({{ $it->id }})" title="Kurangi" @disabled(!$canEdit)><i
                                                        class="bi bi-dash"></i></button>
                                                <input type="number" min="0" class="form-control text-center"
                                                    style="width:50px" value="{{ $it->quantity }}"
                                                    wire:change="updateQuantity({{ $it->id }}, $event.target.value)"
                                                    wire:loading.attr="disabled" @disabled(!$canEdit)
                                                    wire:target="updateQuantity, increment, decrement">
                                                <button class="btn btn-outline-secondary"
                                                    wire:click="increment({{ $it->id }})" title="Tambah" @disabled(!$canEdit)><i
                                                        class="bi bi-plus"></i></button>
                                            </div>
                                        </td>
                                        <td>
                                            <select class="form-select form-select-sm" aria-label="Satuan"
                                                wire:change="updateUnit({{ $it->id }}, $event.target.value)" @disabled(!$canEdit)>
                                                <option value="" @selected(!$it->unit)>(kosong)</option>
                                                @foreach($unitsOptions as $u)
                                                <option value="{{ $u }}" @selected($it->unit===$u)>{{ ucfirst($u) }}
                                                </option>
                                                @endforeach
                                            </select>
                                        </td>
                                        <td class="text-center">
                                            <div class="btn-group btn-group-sm">
                                                <button class="btn btn-outline-primary" wire:click="edit({{ $it->id }})"
                                                    title="Ubah" @disabled(!$canEdit)><i class="bi bi-pencil"></i></button>
                                                <button class="btn btn-outline-danger" title="Hapus" x-data @disabled(!$canEdit)
                                    ...
                                    @empty
                                    <tr>
                                        <td colspan="4" class="text-center p-4 text-muted">Belum ada item inventori di
                                            kamar ini.</td>
                                    </tr>
                                    @endforelse
                                </tbody>
                    ...
                    <div x-show="activeTab === 'actions'" x-transition>
                        <div class="row g-4">
                            <div class="col-md-6">
                                <div class="card h-100">
                                    <div class="card-header bg-white border-bottom">
                                        <h6 class="mb-0"><i class="bi bi-clipboard-plus me-2"></i>Salin Inventori Antar
                                            Kamar</h6>
                                    </div>
                                    <div class="card-body">
                                        <div class="mb-3"><label class="form-label">Salin dari Kamar
                                                ...
                                                <option value="">-- pilih sumber --</option>@foreach($rooms as $r)
                                                <option value="{{ $r->id }}">No. {{ $r->room_number }}</option>
                                                @endforeach
                                            </select></div>
                                        <div class="mb-3"><label class="form-label">Ke Kamar (Tujuan)</label><select
                                                class="form-select" wire:model="targetRoomId">
                                                <option value="">-- pilih tujuan --</option>@foreach($rooms as $r)
                                                <option value="{{ $r->id }}" @disabled($r->id == $sourceRoomId)>No.
                                                    {{ $r->room_number }}</option>@endforeach
                                            </select></div>
                                        <div class="form-check mb-1"><input class="form-check-input" type="checkbox"
                                                id="overwrite" wire:model.live="overwrite"><label
                                                class="form-check-label" for="overwrite">Timpa item di kamar
                                                tujuan</label></div>
                                        <small class="form-text text-muted d-block mb-3">Jika dicentang, semua item di
                                            tujuan akan dihapus sebelum item baru disalin.</small>
                                        <div class="d-grid"><button class="btn btn-info" @disabled(!$sourceRoomId ||
                                                !$targetRoomId) x-data
                                                @click="$wire.dispatch('swal:confirm', { method: 'inventory:copy', title: 'Salin Item?', text: '{{ $overwrite ? 'Item tujuan akan DITIMPA.' : 'Salin komposisi item kamar sumber ke tujuan.' }}', confirmText: 'Ya, salin' })"><i
                                                    class="bi bi-files me-1"></i> Salin ke 1 Kamar</button></div>
                                        @if(!$sourceRoomId || !$targetRoomId)
                                        <small class="text-danger d-block mt-2">Pilih kamar sumber dan satu kamar
                                            tujuan.</small>
                                        @endif
                                    </div>
                                </div>
                            ...
                            <div class="col-md-6">
                                <div class="card h-100">
                                    <div class="card-header bg-white border-bottom">
                                        <h6 class="mb-0"><i class="bi bi-distribute-vertical me-2"></i>Tindakan ke
                                            Banyak Kamar</h6>
                                    </div>
                                    <div class="card-body">
                                        <div class="mb-3">
                                            <label class="form-label">Pilih Kamar Tujuan ({{ count($targetRoomIds) }}
                                                terpilih)</label>
                                            <div class="d-flex gap-2">
                                                ...
                                                <button type="button" class="btn btn-sm btn-outline-primary" wire:click="selectAllFiltered">
                                                    <i class="bi bi-check2-all me-1"></i> Pilih Semua Hasil Filter
                                                </button>
                                            </div>
                                        </div>
                                        <div class="d-grid gap-2">
                                            <button class="btn btn-primary" @disabled(!$sourceRoomId ||
                                                empty($targetRoomIds)) x-data
                                                @click="$wire.dispatch('swal:confirm', { method: 'inventory:copyBulk', title: 'Salin ke {{ count($targetRoomIds) }} Kamar?', text: '{{ $overwrite ? 'Item tujuan akan DITIMPA.' : 'Salin item dari sumber ke semua kamar terpilih.' }}', confirmText: 'Ya, salin' })"><i
                                                    class="bi bi-clipboard2-plus me-1"></i> Salin dari Sumber ke
                                                Terpilih</button>
                                            <button class="btn btn-success" @disabled(!$roomTypeId ||
                                                empty($targetRoomIds)) x-data
                                                ...
                                                Terpilih</button>
                                            <hr class="my-2">
                                            <button class="btn btn-danger" @disabled(!$roomId) x-data @disabled(!$canEdit)
                    ...
                    <div x-show="activeTab === 'template'" x-transition>
                        ...
                        <div class="card border-info">
                            <div
                                ...
                                <h6 class="mb-0"><i class="bi bi-journal-plus me-2"></i>Template Item untuk Tipe Kamar
                                </h6>
                                <span
                                    class="badge bg-light text-dark">{{ optional($types->firstWhere('id', (int)$roomTypeId))->name }}</span>
                            </div>
                            <div class="card-body">
                                <p class="text-muted">Atur item standar untuk tipe kamar ini. Template ini dapat
                                    diterapkan ke banyak kamar sekaligus melalui tab "Tindakan Massal".</p>
                                <div class="row g-2 align-items-end mb-3">
                                    <div class="col-md-4"><label class="form-label">Nama Item</label><input type="text"
                                            class="form-control" wire:model="templateName" placeholder="mis. Shampoo" @disabled(!$canEdit)>
                                    </div>
                                    <div class="col-md-3"><label class="form-label">Jumlah</label><input type="number"
                                            min="0" class="form-control" wire:model="templateQuantity"
                                            placeholder="mis. 2" @disabled(!$canEdit)></div>
                                    <div class="col-md-3"><label class="form-label">Satuan</label><input type="text"
                                            class="form-control" wire:model="templateUnit" placeholder="mis. botol" @disabled(!$canEdit)>
                                    </div>
                                    <div class="col-md-2 d-grid"><button class="btn btn-info"
                                            wire:click="addTemplateItem" wire:loading.attr="disabled" @disabled(!$canEdit)><i
                                                class="bi bi-plus"></i> Tambah</button></div>
                                </div>
                                ...
                                <div class="table-responsive">
                                    <table class="table table-sm table-bordered align-middle">
                                        ...
                                        <tbody>
                                            ...
                                            @empty
                                            <tr>
                                                <td colspan="4" class="text-center p-3 text-muted">Belum ada item
                                                    template.</td>
                                            </tr>
                                            @endforelse
                                        </tbody>
                        ...
                        <div class="alert alert-warning text-center">
                            ...
                            <p class="mb-0">Pilih <strong>Tipe Kamar</strong> pada filter di sebelah kiri untuk
    ...
    <div class="toast show align-items-center text-bg-dark border-0" role="alert" aria-live="assertive" aria-atomic="true">
        <div class="d-flex">
            ...
            <div class="d-flex align-items-center p-2 gap-2">
                ...
                <button type="button" class="btn-close btn-close-white me-2 m-auto" aria-label="Close" wire:click="$set('showUndoBar', false)"></button>
            </div>
        </div>
    </div>
    </div>
...
<div class="modal fade" id="roomPickerModal" tabindex="-1" role="dialog" aria-hidden="true" wire:ignore.self>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="row row-cols-2 row-cols-md-3 g-2">
                    ...
                    @if(!$pickerSearch || str_contains(strtolower((string)$r->room_number),
                    strtolower((string)$pickerSearch)))
                    <div class="col">
                        <label
                            ...
                            <input class="form-check-input" type="checkbox" value="{{ $r->id }}"
                                wire:model.live="targetRoomIds" @disabled($r->id == $sourceRoomId)>
