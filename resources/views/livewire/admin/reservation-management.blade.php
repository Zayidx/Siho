        ...
        <div class="card-body">
            <div class="d-flex mb-4 justify-content-between align-items-center flex-wrap">
                <div class="d-flex gap-2 mb-2 mb-md-0 align-items-center flex-wrap">
                    ...
                    <input type="date" wire:model.live="startDate" class="form-control" style="width:auto;"
                        title="Mulai">
                    <input type="date" wire:model.live="endDate" class="form-control" style="width:auto;"
                        title="Selesai">
                    <input type="search" wire:model.live.debounce.300ms="search" class="form-control"
                        placeholder="Cari nama tamu...">
                </div>
                <div class="d-flex gap-2">
                    <a href="{{ route('admin.reservations.export', ['search' => $search, 'status' => $filterStatus, 'start' => $startDate, 'end' => $endDate]) }}"
            ...
            <div class="table-responsive">
                ...
                <table class="table table-hover table-bordered table-striped">
                    ...
                    <tbody>
                            ...
                            <tr wire:key="{{ $reservation->id }}">
                                ...
                                <td>
                                    ...
                                    @foreach ($reservation->rooms as $room)
                                        <button type="button"
                                            class="btn btn-sm bg-light-secondary text-dark border-0 p-1 me-1 mb-1"
                                ...
                                <td class="text-center">
                                    <div class="d-inline-flex gap-1">
                                        <button class="btn btn-warning btn-sm"
                                            wire:click="edit({{ $reservation->id }})"><i
                                                class="bi bi-pencil-square"></i></button>
                                        <button class="btn btn-danger btn-sm"
                                            wire:click="$dispatch('swal:confirm', { id: {{ $reservation->id }}, method: 'destroy' })"><i
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="text-center py-4">Tidak ada data reservasi yang ditemukan.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
                ...
                <div class="modal-body">
                    <form wire:submit.prevent="store">
                        <div class="btn-group w-100 mb-3" role="group">
                            <button type="button"
                                class="btn {{ !$isCreatingGuest ? 'btn-primary' : 'btn-outline-primary' }}"
                                wire:click="$set('isCreatingGuest', false)">Pilih Tamu yang Ada</button>
                            <button type="button"
                                class="btn {{ $isCreatingGuest ? 'btn-primary' : 'btn-outline-primary' }}"
                                wire:click="$set('isCreatingGuest', true)">Buat Tamu Baru</button>
                        </div>
                            ...
                            <div class="border rounded p-3 mb-3">
                                ...
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        <label for="newGuest_name" class="form-label">Nama Lengkap <span
                                                class="text-danger">*</span></label>
                                        <input type="text"
                                            class="form-control @error('newGuest_name') is-invalid @enderror"
                                            id="newGuest_name" wire:model="newGuest_name">
                                        @error('newGuest_name')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        <label for="newGuest_email" class="form-label">Email <span
                                                class="text-danger">*</span></label>
                                        <input type="email"
                                            class="form-control @error('newGuest_email') is-invalid @enderror"
                                            id="newGuest_email" wire:model="newGuest_email">
                                        @error('newGuest_email')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        <label for="newGuest_phone" class="form-label">Telepon <span
                                                class="text-danger">*</span></label>
                                        <input type="text"
                                            class="form-control @error('newGuest_phone') is-invalid @enderror"
                                            id="newGuest_phone" wire:model="newGuest_phone">
                                        @error('newGuest_phone')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                </div>
                        ...
                        @else
                            <div class="mb-3">
                                <label for="guest_id" class="form-label">Tamu <span
                                        class="text-danger">*</span></label>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <label for="check_in_date" class="form-label">Tanggal Check-in <span
                                        class="text-danger">*</span></label>
                                <input type="date"
                                    class="form-control @error('check_in_date') is-invalid @enderror"
                                    id="check_in_date" wire:model="check_in_date">
                                @error('check_in_date')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-md-6 mb-3">
                                <label for="check_out_date" class="form-label">Tanggal Check-out <span
                                        class="text-danger">*</span></label>
                                <input type="date"
                                    class="form-control @error('check_out_date') is-invalid @enderror"
                                    id="check_out_date" wire:model="check_out_date">
                                ...
                                @forelse($availableRoomTypes as $typeId => $details)
                                    <div class="room-counter">
                                        <div>
                                            ...
                                            <small class="text-muted">{{ $details['available_count'] }} kamar
                                                tersedia</small>
                                        </div>
                                        <div class="room-counter-controls">
                                            <button type="button" class="btn btn-outline-secondary btn-sm"
                                                wire:click="decrementRoomType({{ $typeId }})"
                                                @disabled(($selectedRoomTypes[$typeId] ?? 0) <= 0)>-</button>
                                            <span class="count">{{ $selectedRoomTypes[$typeId] ?? 0 }}</span>
                                            <button type="button" class="btn btn-outline-secondary btn-sm"
                                                wire:click="incrementRoomType({{ $typeId }})"
                                                @disabled(($selectedRoomTypes[$typeId] ?? 0) >= $details['available_count'])>+</button>
                                        </div>
                                    </div>
                                @empty
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <button type="button" class="btn-close" wire:click="closeRoomModal" data-bs-dismiss="modal"
                        aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <table class="table table-borderless table-sm">
                        <tbody>
                            ...
                            <tr>
                                <th>Status</th>
                                <td>: {!! $viewingRoom?->status
                                    ? '<span class="badge ' .
                                        ($viewingRoom->status == 'Available' ? 'bg-success' : 'bg-warning') .
                                        '">' .
                                        e($viewingRoom->status) .
                                        '</span>'
                                    : '-' !!}</td>
                            </tr>
