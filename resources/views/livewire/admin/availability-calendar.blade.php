    ...
    <div class="card-header">
        <div class="d-flex flex-wrap gap-3 align-items-center justify-content-between">
            ...
            <div class="d-flex align-items-center gap-3 small">
                <span class="d-inline-flex align-items-center gap-1"><span class="rounded-circle d-inline-block"
                        style="width:12px;height:12px;background:#0d6efd"></span> Booked</span>
                <span class="d-inline-flex align-items-center gap-1"><span class="rounded-circle d-inline-block"
                        style="width:12px;height:12px;background:#198754"></span> Checked-in</span>
                <span class="d-inline-flex align-items-center gap-1"><span class="rounded-circle d-inline-block"
                        style="width:12px;height:12px;background:#6c757d"></span> Checked-out</span>
                <span class="d-inline-flex align-items-center gap-1"><span class="rounded-circle d-inline-block"
                        style="width:12px;height:12px;background:#dc3545"></span> Cancelled</span>
            </div>
        </div>
        <div class="mt-3 row g-2 align-items-end">
            ...
            <div class="col-12 col-md-3">
                ...
                <input type="text" class="form-control form-control-sm" placeholder="cth: 101"
                    wire:model.debounce.400ms="roomNumber" wire:change="filtersUpdated">
            </div>
            <div class="col-12 col-md-3 d-flex gap-2">
                <button class="btn btn-sm btn-outline-secondary" wire:click="filtersUpdated">Terapkan</button>
                <button class="btn btn-sm btn-light"
                    onclick="window.calendarFilters={status:'',room_type_id:'',room:''}; if(window.calendar){ window.calendar.refetchEvents(); }">Reset</button>
            </div>
        </div>
    </div>
...
@push('scripts')
    ...
    <script>
        document.addEventListener('livewire:init', function() {
            ...
            document.addEventListener('keydown', function(e) {
                if (e.key === 'Escape') {
                    document.getElementById('eventOverlay')?.classList.add('d-none');
                }
            });
    ...
    <div id="eventOverlay" class="position-fixed top-0 start-0 w-100 h-100 d-none"
        style="background: rgba(0,0,0,.65); z-index: 1080;">
        <div id="eventOverlayBox" class="bg-dark rounded shadow p-3"
            style="position:absolute; left:50%; top:10%; transform:translateX(-50%); width: min(560px, 92vw);"></div>
