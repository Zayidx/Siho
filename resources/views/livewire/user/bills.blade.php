<div class="container py-4">
    @include('components.public.breadcrumbs', [
        'items' => [['label' => 'Dashboard', 'url' => route('user.dashboard')], ['label' => 'Tagihan']],
    ])
    ...
    <div class="card mb-4">
        <div class="card-body">
            <ul class="nav nav-pills mb-2">
                <li class="nav-item"><a class="nav-link {{ $status === '' ? 'active' : '' }}" href="#"
                        wire:click.prevent="$set('status','')">Semua</a></li>
                <li class="nav-item"><a class="nav-link {{ $status === 'unpaid' ? 'active' : '' }}" href="#"
                        wire:click.prevent="$set('status','unpaid')">Unpaid</a></li>
                <li class="nav-item"><a class="nav-link {{ $status === 'pending' ? 'active' : '' }}" href="#"
                        wire:click.prevent="$set('status','pending')">Pending</a></li>
                <li class="nav-item"><a class="nav-link {{ $status === 'rejected' ? 'active' : '' }}" href="#"
                        wire:click.prevent="$set('status','rejected')">Rejected</a></li>
                <li class="nav-item"><a class="nav-link {{ $status === 'paid' ? 'active' : '' }}" href="#"
                        wire:click.prevent="$set('status','paid')">Paid</a></li>
            </ul>
    ...
    <div class="row g-3">
                    ...
                    <div class="card-body d-flex flex-column">
                        <div class="d-flex justify-content-between align-items-start">
                            <div>
                                ...
                                <div class="text-muted small">Terbit:
                                    {{ optional($bill->issued_at)->format('Y-m-d H:i') }}</div>
                                    ...
                                    @if (!$bill->payment_proof_path || $bill->payment_review_status === 'rejected')
                                        ...
                                        <div class="d-flex gap-2">
                                            <input type="file" class="form-control form-control-sm"
                                                wire:model="proofFile" accept=".jpg,.jpeg,.png,.pdf">
                                            <button class="btn btn-sm btn-outline-primary"
                                                wire:click="uploadProof({{ $bill->id }})"
                                    ...
                                    @else
                                        <div class="small text-muted mt-2">
                                            ...
                                            @if ($proof && Storage::disk('public')->exists($proof))
                                                <a href="#"
                                                    wire:click.prevent="openPreviewBill({{ $bill->id }})">Lihat</a>
                                            @else
                                                <span class="text-muted">(file tidak ditemukan)</span>
                                            @endif
                                            · Status:
                                            <strong>{{ strtoupper($bill->payment_review_status ?? 'pending') }}</strong>
                                        </div>
                                    @endif
                                </div>
                            </div>
                        @else
                            <div class="mt-3">
                                <a href="{{ route('user.bills.invoice', ['bill' => $bill->id]) }}" target="_blank"
                                    class="btn btn-sm btn-outline-secondary">Unduh Invoice (PDF)</a>
                            </div>
                        @endif
                                        ...
                                        <li class="d-flex justify-content-between align-items-start">
                                            <span>
                                                ...
                                                @if ($log->action === 'proof_uploaded')
                                                    · <a href="#"
                                                        wire:click.prevent="openPreviewBill({{ $bill->id }})">Lihat
                                                        bukti</a>
                                                @endif
                                            </span>
        ...
        @empty
            <div class="col-12">
                <div class="alert alert-warning">Tidak ada tagihan.</div>
            </div>
        @endforelse
    </div>
    ...
    @if ($previewUrl)
        <div class="position-fixed top-0 start-0 w-100 h-100" style="background: rgba(0,0,0,0.85); z-index: 1080;">
            <button class="btn btn-light position-absolute" style="top:16px; right:16px;"
                wire:click="closePreview">Tutup</button>
            <div class="d-flex align-items-center justify-content-center h-100 p-3">
                <iframe src="{{ $previewUrl }}" style="width: 90%; height: 90%; background:#fff; border-radius:6px;"
            ...
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <div><strong>Total:</strong> Rp
                                    {{ number_format($selectedBill->total_amount, 0, ',', '.') }}</div>
                                <div><strong>Diterbitkan:</strong>
                                    {{ optional($selectedBill->issued_at)->format('Y-m-d H:i') }}</div>
                                <div><strong>Dibayar:</strong>
                                    {{ optional($selectedBill->paid_at)->format('Y-m-d H:i') ?? '-' }}</div>
                                <div><strong>Metode:</strong> {{ $selectedBill->payment_method ?? '-' }}</div>
                                @php($proof = $selectedBill->payment_proof_path)
                                @if ($proof && Storage::disk('public')->exists($proof))
                                    <div><strong>Bukti:</strong> <a href="{{ Storage::url($proof) }}"
                            ...
                            <div class="col-md-6">
                                ...
                                <div><strong>Check-in:</strong>
                                    {{ $selectedBill->reservation->check_in_date->format('Y-m-d') }}</div>
                                <div><strong>Check-out:</strong>
                                    {{ $selectedBill->reservation->check_out_date->format('Y-m-d') }}</div>
                            ...
                            <div class="mt-3">
                                @if (!$selectedBill->payment_proof_path || $selectedBill->payment_review_status === 'rejected')
                                    ...
                                    <div class="mt-2 d-flex align-items-center gap-2">
                                        ...
                                        <button class="btn btn-outline-primary"
                                            wire:click="uploadProof({{ $selectedBill->id }})"
                                ...
                                @else
                                    <div class="alert alert-info mt-2">
                                        ...
                                        {{ optional($selectedBill->payment_proof_uploaded_at)->format('Y-m-d H:i') }}.
                                        <a href="{{ Storage::url($selectedBill->payment_proof_path) }}"
                                            target="_blank">Lihat bukti</a>
                                        · Status:
                                        <strong>{{ strtoupper($selectedBill->payment_review_status ?? 'pending') }}</strong>
                                    </div>
                                @endif
                                <div class="mt-2 small text-muted">
                                    Transfer ke <strong>{{ config('payment.bank.name') }}</strong> a.n
                                    <strong>{{ config('payment.bank.holder') }}</strong> no. rek
                                    <strong>{{ implode(' ', str_split(config('payment.bank.account'), 4)) }}</strong>.
                        ...
                        @else
                            <div class="mt-3">
                                <a href="{{ route('user.bills.invoice', ['bill' => $selectedBill->id]) }}"
                                    class="btn btn-outline-secondary" target="_blank">Unduh Invoice (PDF)</a>
                            </div>
                        @endif
                    </div>
