            ...
            <div class="row" wire:poll.10s>
                <div class="col-md-6">
                    <ul class="list-unstyled mb-0">
                        ...
                        <li><strong>Kamar:</strong> {{ $reservation->rooms->pluck('room_number')->join(', ') ?: '-' }}
                        </li>
                    </ul>
                                    ...
                                    <div class="list-group-item">
                                        <div class="d-flex justify-content-between align-items-center">
                                            <div>
                                                ...
                                                <div class="small text-muted">Jumlah: {{ $t['count'] }} kamar
                                                    @if ($t['capacity'])
                                                        · Kapasitas: {{ $t['capacity'] }} org/kamar
                                                    @endif
                                                </div>
                                            </div>
                                            <div class="text-end">
                                                ...
                                                <div class="fw-semibold">Rp
                                                    {{ number_format($t['avg_price'], 0, ',', '.') }}</div>
                                            </div>
                                        </div>
                ...
                <div class="col-md-6">
                    ...
                    @if ($reservation->bill)
                        <div>Total: <strong>Rp
                                {{ number_format($reservation->bill->total_amount, 0, ',', '.') }}</strong></div>
                        <div>Status: {{ $reservation->bill->paid_at ? 'Paid' : 'Unpaid' }}</div>
                        @if ($reservation->bill->paid_at)
                            <div class="mt-2">
                                <a class="btn btn-outline-secondary btn-sm"
                                    href="{{ route('user.bills.invoice', ['bill' => $reservation->bill->id]) }}"
                                    target="_blank">Unduh Invoice (PDF)</a>
                            </div>
                        @else
                            <div class="mt-3">
                                @if (!$reservation->bill->payment_proof_path || $reservation->bill->payment_review_status === 'rejected')
                                    ...
                                    <div class="d-flex gap-2 align-items-center">
                                        ...
                                        <button class="btn btn-primary btn-sm" wire:click="uploadProof"
                                ...
                                @else
                                    <div class="alert alert-info">
                                        Bukti sudah diunggah pada
                                        {{ optional($reservation->bill->payment_proof_uploaded_at)->format('Y-m-d H:i') }}.
                                        <a href="#" wire:click.prevent="openPreview">Lihat bukti</a> · Status:
                                        <strong>{{ strtoupper($reservation->bill->payment_review_status ?? 'pending') }}</strong>
                                    </div>
                                @endif
                            </div>
                        @endif
