    ...
    @if (!$showOtpForm)
        ...
        <form wire:submit='submitRegistrationDetails' novalidate>
            ...
            <div class="mb-3 form-group position-relative has-icon-left">
                <input required type="text" wire:model.blur='full_name'
            ...
            <div class="mb-3 form-group position-relative has-icon-left">
                <input required type="text" wire:model.blur='username'
            ...
            <div class="mb-3 form-group position-relative has-icon-left">
                <input required type="text" wire:model.blur='phone'
            ...
            <div class="mb-3">
                ...
                <div class="row g-3">
                            ...
                            <template x-if="!manualMode && !hasError">
                                <div>
                                    <div class="form-group position-relative has-icon-left">
                                        <select
                                            class="form-select form-select-xl @error('province') is-invalid @enderror"
                                    ...
                                    <div class="form-group position-relative has-icon-left mt-3">
                                        <select class="form-select form-select-xl @error('city') is-invalid @enderror"
                                            x-model="selectedCityId"
                                            @change="$wire.set('city_id', selectedCityId); const c = cities.find(ct => ct.id == selectedCityId); $wire.set('city', c ? c.name : '');"
                            ...
                            <template x-if="manualMode || hasError">
                                <div>
                                    <div class="form-group position-relative has-icon-left mb-3">
                                        <input type="text"
                                            class="form-control form-control-xl @error('province') is-invalid @enderror"
                                            placeholder="Provinsi" @input="$wire.set('province', $event.target.value)">
                    ...
                    <div class="col-md-6">
                        <div class="form-group position-relative has-icon-left mb-3">
                            <input required type="text" wire:model.blur='street'
                                class="form-control form-control-xl @error('street') is-invalid @enderror"
                        ...
                        <div class="row g-3">
                            <div class="col-6">
                                <div class="form-group position-relative has-icon-left">
                                    <input type="text" wire:model.blur='rt'
                                        class="form-control form-control-xl @error('rt') is-invalid @enderror"
                                        placeholder="RT">
                            ...
                            <div class="col-6">
                                <div class="form-group position-relative has-icon-left">
                                    <input type="text" wire:model.blur='rw'
                                        class="form-control form-control-xl @error('rw') is-invalid @enderror"
                                        placeholder="RW">
                                    <div class="form-control-icon"><i class="bi bi-123"></i></div>
                                    @error('rw')
                                        <div class="invalid-feedback">{{ $message }}</div>
            ...
            <div class="mb-4 form-group position-relative has-icon-left">
                <input required type="password" wire:model.blur='password'
            ...
            <div class="mb-4 form-group position-relative has-icon-left">
                <input required type="password" wire:model.blur='password_confirmation'
            ...
            <div class="mb-3 form-group">
                ...
                <input type="file" class="form-control @error('foto') is-invalid @enderror" id="foto"
                    wire:model="foto" accept="image/jpeg,image/png,image/webp">
                @error('foto')
                    <div class="invalid-feedback"><i class="bx bx-radio-circle"></i> {{ $message }}</div>
                @enderror
                <div wire:loading wire:target="foto" class="text-muted mt-1 small">Mengunggah...</div>
                @if ($foto)
                    <img src="{{ $foto->temporaryUrl() }}" class="img-thumbnail mt-2" style="max-height: 150px;"
                        alt="Preview">
                @endif
            </div>
            ...
            <button type="submit" class="mt-3 shadow-lg btn btn-primary btn-block btn-lg"
                ...
                <span wire:loading wire:target='submitRegistrationDetails' class="spinner-border spinner-border-sm"
                    role="status" aria-hidden="true"></span>
            </button>

            <p class="m-0 mt-5 text-xl text-center text-dark">Sudah punya akun? <a href="{{ route('login') }}"
                    class="text-primary fw-bold text-decoration-underline" wire:navigate>Login di sini</a></p>
        </form>
    @else
        {{-- TAMPILAN FORM VERIFIKASI OTP --}}
        <h1 class="mt-5 auth-title">Verifikasi Email Anda</h1>
        <p class="mb-5 auth-subtitle pe-5">Kami telah mengirimkan kode OTP ke email
            <strong>{{ $email }}</strong>. Satu langkah lagi!
        </p>
        ...
        <div x-data="{
        ...
        }" x-init="startTimer()" @otp-sent.window="startTimer()">
            ...
            <form wire:submit='verifyOtpAndCreateUser'>
                ...
                <div class="mb-4 form-group position-relative has-icon-left">
                    <input required type="text" inputmode="numeric" pattern="[0-9]*" maxlength="6"
                        wire:model.blur='otp' class="form-control form-control-xl @error('otp') is-invalid @enderror"
                ...
                <button type="submit" class="mt-3 shadow-lg btn btn-primary btn-block btn-lg"
                    ...
                    <span wire:loading wire:target='verifyOtpAndCreateUser' class="spinner-border spinner-border-sm"
                        role="status" aria-hidden="true"></span>
                </button>
            </form>
            ...
            <div class="mt-4 text-center">
                <template x-if="timerRunning">
                    <p class="text-muted">Kirim ulang kode dalam <span x-text="formattedTime" class="fw-bold"></span>
                    </p>
                </template>
                <template x-if="!timerRunning">
                    <p class="text-muted">Tidak menerima kode? <a href="#" wire:click.prevent="resendOtp"
                            class="font-bold">Kirim Ulang</a></p>
                </template>
                <a href="#" wire:click.prevent="cancelOtp" class="text-muted d-block mt-2">Salah data?
                    Kembali</a>
            </div>
        </div>
    @endif
