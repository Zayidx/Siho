    ...
    <div class="container px-4 mx-auto">
        ...
        <div class="grid max-w-5xl gap-8 mx-auto md:grid-cols-2 lg:grid-cols-3">
            @if(($roomTypeSummaries ?? collect())->count() > 0)
                        ...
                        <div class="px-4 mt-2 flex gap-4 overflow-x-auto">
                            @foreach($imgs as $ix => $img)
                                ...
                                <div class="flex flex-col items-center gap-1">
                                    <img src="{{ $img['url'] ?? '' }}" alt="thumb" title="{{ $type['name'] }} - {{ !empty($img['category']) ? ucfirst($img['category']) : 'Foto' }}" class="w-14 h-14 rounded object-cover fs-img cursor-zoom-in" loading="lazy" decoding="async" onerror="this.onerror=null;this.src='https://placehold.co/160x160/777/FFF?text=Foto';">
                ...
                @endforeach
            @else
                <div class="flex flex-col text-center bg-white rounded-lg shadow-sm dark:bg-gray-800 border border-gray-200 dark:border-gray-700 p-8">
