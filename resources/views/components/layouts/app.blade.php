...
<head>
    ...
    <title>{{ $title ?? 'Dashboard' }} - {{ config('app.name', 'Hotel Grand Luxe') }}</title>
...
<body>
    ...
    <div id="app">
        <div id="sidebar">
            <div class="sidebar-wrapper active">
                ...
                @include('components.layouts.partials.sidebar_superadmin')
            </div>
        </div>
        <div id="main">
            <header class="mb-3">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <div class="d-none d-md-flex align-items-center gap-2">
                        <a href="{{ route('admin.payments') }}"
                            ...
                            @if ($pendingPayments)
                                <span
                                    class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger">{{ $pendingPayments }}</span>
                            @endif
                        </a>
                        <a href="{{ route('admin.contacts') }}"
                            ...
                            @if ($unreadContacts)
                                <span
                                    class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger">{{ $unreadContacts }}</span>
                            @endif
                        </a>
                    </div>
                </div>
    ...
    {{-- Script Listener SweetAlert dengan Tema Gelap --}}
    <script>
        ...
        document.addEventListener('livewire:init', () => {
            ...
            const showModalById = (id) => {
                ...
                if (ensureBootstrap()) {
                    ...
                    try {
                        prevFocus.set(id, document.activeElement);
                    } catch (e) {}
            ...
            const hideModalById = (id) => {
                ...
                if (ensureBootstrap()) {
                    ...
                    requestAnimationFrame(() => {
                        ...
                        document.querySelectorAll('.modal-backdrop[data-temp]')?.forEach(n => n
                            .remove());
