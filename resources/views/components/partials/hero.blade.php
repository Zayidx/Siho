        ...
        @push('scripts')
            <script>
                (function() {
                    ...
                    function validate() {
                        const a=ci.value; const b=co.value; if(!a||!b){ err.classList.add('hidden'); return true; }
                        const da=new Date(a); const db=new Date(b); if(isNaN(da)||isNaN(db)){ err.classList.remove('hidden'); (isNaN(da)?ci:co).focus(); return false; }
                        const ok=db>da; err.classList.toggle('hidden', ok); if(!ok){ try{ co.focus(); }catch(e){} } return ok;
                    }
                    ...
                    function updateCtas(){ const base1=cta1?.getAttribute('data-base')||''; const base2=cta2?.getAttribute('data-base')||''; const params=new URLSearchParams(); if(ci?.value) params.set('checkin', ci.value); if(co?.value) params.set('checkout', co.value); const qs=params.toString(); if(cta1 && base1) cta1.href = qs?`${base1}?${qs}`:base1; if(cta2 && base2) cta2.href = qs?`${base2}?${qs}`:base2; }
                    function applyStoredIfNoQuery(){ const urlParams=new URLSearchParams(window.location.search); if(urlParams.has('checkin')||urlParams.has('checkout')) return; const today=new Date(); today.setHours(0,0,0,0); let sIn=localStorage.getItem(kIn); let sOut=localStorage.getItem(kOut); if(sIn){ const dIn=new Date(sIn); if(!isNaN(dIn) && dIn>today) ci.value=sIn; } if(sOut){ const dOut=new Date(sOut); const dIn=new Date(ci.value); if(!isNaN(dOut) && !isNaN(dIn) && dOut>dIn) co.value=sOut; } }
